...
const getTopSpenders = async (req, res, next) => {
  try {
    ...
    const customerIds = Array.from(spendingMap.keys()).map(
...
const getUsersWithRevenue = async (req, res, next) => {
  try {
    ...
    const enrichedUsers = customers.map((user) => {
      ...
      const spends = spendingMap.get(idStr) || {
        spentOnProducts: 0,
        spentOnEvents: 0,
      };

      const totalSpent = spends.spentOnProducts + spends.spentOnEvents;
      const revenue = totalSpent * 0.1; // 10% commission
...
const getUser = async (req, res, next) => {
  try {
    ...
    const user = await Customer.findById(userId).select(
      "_id userName email phoneNumber address profilePic createdAt",
    );
...
const updateUser = async (req, res, next) => {
  try {
    const userId = req.params.id;
    const { userName, phoneNumber, houseNumber, streetNo, city, pincode } =
      req.body;

    const user_address =
      houseNumber || streetNo || city || pincode
        ? { houseNumber, streetNo, city, pincode }
        : undefined;
    ...
    await Customer.updateOne({ _id: userId }, updateData);
...
const getProductData = async (req, res, next) => {
  try {
    ...
    const result =
      metrics.length > 0
        ? metrics[0]
        : { totalSales: 0, revenue: 0, uniqueCustomers: 0 };
...
const getUserStats = async (req, res, next) => {
  try {
    ...
    const monthly = await Customer.countDocuments({
      createdAt: { $gte: monthAgo },
    });
    const weekly = await Customer.countDocuments({
...
const getProductStats = async (req, res, next) => {
  try {
    ...
    const totalLastMonth = await Product.countDocuments({
    ...
    const inStockLastMonth =
      inStockLastMonthResult.length > 0
        ? inStockLastMonthResult[0].inStockLastMonth
        : 0;
    ...
    const lowStockLastWeek =
      lowStockLastWeekResult.length > 0
        ? lowStockLastWeekResult[0].lowStockLastWeek
        : 0;
    ...
    const outOfStock =
      outOfStockResult.length > 0 ? outOfStockResult[0].outOfStock : 0;
    ...
    const outOfStockYesterday =
      outOfStockYesterdayResult.length > 0
        ? outOfStockYesterdayResult[0].outOfStockYesterday
        : 0;
...
const dashBoardStats = async (req, res, next) => {
  try {
    ...
    const totalRevenueOrdersValue =
      totalRevenueOrders.length > 0 ? totalRevenueOrders[0].total : 0;
    ...
    const totalRevenueEventsValue =
      totalRevenueEvents.length > 0 ? totalRevenueEvents[0].total : 0;

    const totalRevenue =
      (totalRevenueOrdersValue + totalRevenueEventsValue) * 0.1;
    ...
    const monthlyRevenueEventsValue =
      monthlyRevenueEvents.length > 0 ? monthlyRevenueEvents[0].total : 0;

    const monthlyRevenueOrders = await Order.aggregate([
      { $match: { order_date: { $gte: monthAgo } } },
      { $group: { _id: null, total: { $sum: "$subtotal" } } },
    ]);
    const monthlyRevenueOrdersValue =
      monthlyRevenueOrders.length > 0 ? monthlyRevenueOrders[0].total : 0;

    const monthlyRevenue =
      (monthlyRevenueOrdersValue + monthlyRevenueEventsValue) * 0.1;
    ...
    const weeklyRevenueEventsValue =
      weeklyRevenueEvents.length > 0 ? weeklyRevenueEvents[0].total : 0;

    const weeklyRevenueOrders = await Order.aggregate([
      { $match: { order_date: { $gte: weekAgo } } },
      { $group: { _id: null, total: { $sum: "$subtotal" } } },
    ]);
    const weeklyRevenueOrdersValue =
      weeklyRevenueOrders.length > 0 ? weeklyRevenueOrders[0].total : 0;

    const weeklyRevenue =
      (weeklyRevenueOrdersValue + weeklyRevenueEventsValue) * 0.1;
    ...
    const dailyRevenueEventsValue =
      dailyRevenueEvents.length > 0 ? dailyRevenueEvents[0].total : 0;

    const dailyRevenueOrders = await Order.aggregate([
      { $match: { order_date: { $gte: today } } },
      { $group: { _id: null, total: { $sum: "$subtotal" } } },
    ]);
    const dailyRevenueOrdersValue =
      dailyRevenueOrders.length > 0 ? dailyRevenueOrders[0].total : 0;

    const dailyRevenue =
      (dailyRevenueOrdersValue + dailyRevenueEventsValue) * 0.1;
...
const getRevenueChartData = async (req, res, next) => {
  try {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const eventRevenueValue =
        eventRevenue.length > 0 ? eventRevenue[0].total : 0;
  ...
  try {
    ...
    const totalRevenueAgg = await OrderItem.aggregate([
      ...
      { $unwind: "$order" },
      {
        $group: {
          _id: "$product.vendor_id",
          totalSubtotal: { $sum: "$order.subtotal" },
        },
      },
      { $group: { _id: null, totalRevenue: { $sum: "$totalSubtotal" } } },
    ]);
    const totalRevenue =
      totalRevenueAgg.length > 0 ? totalRevenueAgg[0].totalRevenue * 0.1 : 0;
    ...
    const totalOrders =
      totalOrdersAgg.length > 0 ? totalOrdersAgg[0].totalOrders : 0;
    ...
    const todaysOrders =
      todaysOrdersAgg.length > 0 ? todaysOrdersAgg[0].todaysOrders : 0;
    ...
    const yesterdayOrders =
      yesterdayOrdersAgg.length > 0 ? yesterdayOrdersAgg[0].yesterdayOrders : 0;
    const todaysOrdersChange =
      yesterdayOrders > 0
    ...
    const lastMonthAgg = await OrderItem.aggregate([
      ...
      },
      {
        $group: {
          _id: null,
          lastMonthOrders: { $sum: "$lastMonthCount" },
          lastMonthRevenue: { $sum: "$lastMonthSubtotal" },
        },
      },
    ]);
    const lastMonthOrders =
      lastMonthAgg.length > 0 ? lastMonthAgg[0].lastMonthOrders : 0;
    const lastMonthRevenue =
      lastMonthAgg.length > 0 ? lastMonthAgg[0].lastMonthRevenue * 0.1 : 0;
    ...
    const thisMonthAgg = await OrderItem.aggregate([
      ...
      },
      {
        $group: {
          _id: null,
          thisMonthOrders: { $sum: "$thisMonthCount" },
          thisMonthRevenue: { $sum: "$thisMonthSubtotal" },
        },
      },
    ]);
    const thisMonthOrders =
      thisMonthAgg.length > 0 ? thisMonthAgg[0].thisMonthOrders : 0;
    const thisMonthRevenue =
      thisMonthAgg.length > 0 ? thisMonthAgg[0].thisMonthRevenue * 0.1 : 0;
...
const getVendorRevenueMetrics = async (req, res, next) => {
  try {
    ...
    const monthlyBreakdown = await OrderItem.aggregate([
      ...
      },
      {
        $group: {
          ...
          month: {
            $first: {
              $dateToString: { format: "%Y-%m", date: "$order.order_date" },
            },
          },
    ...
    res.json({
      ...
      metrics: {
        today_revenue:
          revenueMetrics.length > 0 ? revenueMetrics[0].today_revenue : 0,
        weekly_revenue:
          revenueMetrics.length > 0 ? revenueMetrics[0].weekly_revenue : 0,
        monthly_revenue:
          revenueMetrics.length > 0 ? revenueMetrics[0].monthly_revenue : 0,
        total_revenue:
          revenueMetrics.length > 0 ? revenueMetrics[0].total_revenue : 0,
...
/* ─────────────────────────────────────────────────────────────
   GET VENDOR ORDERS BY DATE RANGE (For Admin Date Filter)
   ───────────────────────────────────────────────────────────── */
const getVendorOrdersByDateRange = async (req, res, next) => {
  try {
    const vendorId = req.params.id;
    ...
    // Fetch orders with order items for this vendor in date range
    const orders = await OrderItem.aggregate([
      ...
      ...(Object.keys(dateFilter).length > 0
        ? [{ $match: { "order.order_date": dateFilter } }]
        : []),
      {
        $lookup: {
          from: "customers",
      ...
      { $unwind: { path: "$customer", preserveNullAndEmptyArrays: true } },
      {
        $project: {
          ...
          status: "$order.status",
          customer_name: { $ifNull: ["$customer.userName", "N/A"] },
          customer_email: { $ifNull: ["$customer.email", "N/A"] },
          product_name: "$product_name",
          quantity: "$quantity",
          price: "$price",
          item_total: { $multiply: ["$quantity", "$price"] },
          vendor_revenue: {
            $multiply: [{ $multiply: ["$quantity", "$price"] }, 0.92],
          },
        },
      },
      { $sort: { order_date: -1 } },
    ...
    const revenueResult = await OrderItem.aggregate([
        ...
        : []),
      {
        $group: {
          _id: null,
          total_revenue: {
            $sum: { $multiply: [{ $multiply: ["$quantity", "$price"] }, 0.92] },
          },
          total_items: { $sum: "$quantity" },
          total_orders: { $addToSet: "$order._id" },
        },
      },
      {
        $project: {
          ...
          total_revenue: 1,
          total_items: 1,
          total_unique_orders: { $size: "$total_orders" },
        },
      },
    ]);
    ...
    const summary =
      revenueResult.length > 0
        ? revenueResult[0]
        : { total_revenue: 0, total_items: 0, total_unique_orders: 0 };
    ...
    console.error("Error fetching vendor orders by date range:", err);
    next(err);
  }
};
...
const getVendor = async (req, res, next) => {
  try {
    ...
    const vendor = await Vendor.findById(vendorId).select(
      "_id name email contact_number store_name store_location created_at",
    );
...
const getEventManagerMetrics = async (req, res, next) => {
  try {
    ...
    const [upcoming, weekly, monthly, totalRevenueResult, monthlyBreakdown] =
      await Promise.all([
        Event.countDocuments({ eventManagerId: id, date_time: { $gte: now } }),
        Event.countDocuments({
          eventManagerId: id,
          date_time: { $gte: startOfWeek },
        }),
        Event.countDocuments({
          eventManagerId: id,
          date_time: { $gte: startOfMonth },
        }),
        ...
        Ticket.aggregate([
          ...
          {
            $match: {
              "event.eventManagerId": new mongoose.Types.ObjectId(id),
              status: true,
            },
          },
          { $group: { _id: null, total: { $sum: "$price" } } },
...
/* ─────────────────────────────────────────────────────────────
   GET EVENT MANAGER EVENTS BY DATE RANGE (For Admin Date Filter)
   ───────────────────────────────────────────────────────────── */
const getEventManagerEventsByDateRange = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { startDate, endDate } = req.query;
    ...
    // Fetch events for this event manager in date range
    const events = await Event.find({
      eventManagerId: id,
      ...(Object.keys(dateFilter).length > 0 ? { date_time: dateFilter } : {}),
    })
      .select(
        "title date_time venue location total_tickets tickets_sold ticketPrice category",
      )
      .sort({ date_time: -1 });

    // Format events with revenue calculation
    const formatted = events.map((e) => {
      // Revenue = (tickets_sold * ticketPrice)
      const totalRevenue = (e.tickets_sold || 0) * (e.ticketPrice || 0);
      return {
        event_id: e._id.toString(),
        event_name: e.title,
        date: e.date_time,
    ...
    // Calculate summary
    const totalRevenue = formatted.reduce((sum, e) => sum + e.total_revenue, 0);
    const totalEvents = formatted.length;
    const totalTicketsSold = formatted.reduce(
    ...
    console.error("Error fetching event manager events by date range:", error);
    next(error);
  }
};
...
const updateEventManager = async (req, res) => {
  try {
    ...
    const manager = await EventManager.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    }).select("-password");
...
const deleteEventManager = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to delete manager" });
  }
};
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      for (const [index, file] of req.files.entries()) {
        let imageUrl;
        try {
          imageUrl = await uploadToCloudinary(file, "product_images", [
...
const getEventsData = async (req, res) => {
  try {
    ...
    const ticketsSold =
      ticketAggregation.length > 0 ? ticketAggregation[0].totalTickets : 0;
...
const getEventAttendees = async (req, res) => {
  try {
    ...
    const attendees = await Ticket.find({ eventId: eventId })
      ...
      .select(
        "_id ticketId numberOfTickets purchaseDate status price petName petBreed petAge",
      );
...
const updateEvent = async (req, res) => {
  try {
    ...
    // Handle Thumbnail
    let finalThumbnail =
      existing_thumbnail || (event.images ? event.images.thumbnail : null);
    if (thumbnailFile) {
      try {
    ...
    // Handle Banner
    let finalBanner =
      existing_banner || (event.images ? event.images.banner : null);
    if (bannerFile) {
      try {
        finalBanner = await uploadToCloudinary(bannerFile, "event_banners", [
        ...
        return res
          .status(500)
          .json({ success: false, message: "Failed to upload event banner." });
      }
    }
    ...
    await Event.updateOne({ _id: eventId }, updateData);
...
const getOrderDetails = async (req, res) => {
  try {
    ...
    const customerAddress =
      customer && customer.address
        ? `${customer.address.houseNumber || ""}, ${customer.address.streetNo || ""}, ${customer.address.city || ""} - ${customer.address.pincode || ""}`
            .trim()
            .replace(/,\s*,\s*/g, ", ")
            .replace(/^,\s*|,\s*$/g, "")
        : "N/A";
      ...
      order: {
        ...
        items: orderItems.map((item) => ({
          ...
          productName: item.product_id
            ...
            : item.product_name,
          vendorName:
            item.product_id && item.product_id.vendor_id
              ? item.product_id.vendor_id.store_name
              : "N/A",
          price: item.price,
...
const getEventRevenue = async (req, res) => {
  try {
    ...
    const thisMonthSales =
      thisMonthAgg.length > 0 ? thisMonthAgg[0].thisMonthSales : 0;
    ...
    const lastMonthSales =
      lastMonthAgg.length > 0 ? lastMonthAgg[0].lastMonthSales : 0;
