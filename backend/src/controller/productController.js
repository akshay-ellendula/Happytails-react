...
const buildValidatedCart = async (cart) => {
  ...
  const productIds = [
    ...new Set(normalizedCart.map((item) => item.product_id)),
  ];
  const variantIds = [
    ...new Set(normalizedCart.map((item) => item.variant_id)),
  ];
  ...
  const [products, variants] = await Promise.all([
    ...
    ProductVariant.find({ _id: { $in: variantIds } })
      .select(
        "_id product_id size color regular_price sale_price stock_quantity",
      )
      .lean(),
  ]);
  ...
  const cleanCart = normalizedCart.map((item) => {
    ...
    if (
      !product ||
      !variant ||
      variant.product_id.toString() !== item.product_id
    ) {
    ...
    if (variant.stock_quantity < item.quantity) {
      throw new Error(
        `Only ${variant.stock_quantity} item(s) left for ${product.product_name}`,
      );
    }
    ...
    return {
      ...
      vendor_id: product.vendor_id?.toString() || null,
        ...
        Product.aggregate([
          ...
          },
          {
            $project: {
              ...
              product_type: {
                $ifNull: [
                  { $toLower: { $trim: { input: "$product_type" } } },
                  "unknown",
                ],
              },
              product_category: 1,
              variants: {
                $map: {
                  ...
                  in: {
                    ...
                    size: {
                      $ifNull: [
                        { $toLower: { $trim: { input: "$$variant.size" } } },
                        null,
                      ],
                    },
                    color: {
                      $ifNull: [
                        { $toLower: { $trim: { input: "$$variant.color" } } },
                        null,
                      ],
                    },
...
const getProduct = async (req, res, next) => {
  try {
    ...
    const product = await Product.findById(productId).select(
      "id product_name product_type product_category product_description",
    );
    if (!product)
      return res
        .status(404)
    ...
    if (sessionData.paymentIntentId) {
      try {
        ...
        const metadataMatches =
          existingPaymentIntent.metadata?.checkoutType === "product" &&
          existingPaymentIntent.metadata?.customerId ===
            req.user.customerId.toString() &&
          existingPaymentIntent.metadata?.checkoutSessionId ===
            checkoutSessionId;
        const amountMatches =
          existingPaymentIntent.amount === amountInPaise &&
        ...
        console.warn(
          "Unable to reuse existing payment intent:",
          stripeError.message,
        );
      }
    }
  ...
  try {
    ...
    if (!sessionPaymentIntentId || sessionPaymentIntentId !== paymentIntentId) {
      return res
        ...
        .json({
          success: false,
          message: "Payment intent does not match the active checkout session",
        });
    }
    ...
    const paymentMatchesCheckout =
      paymentIntent.metadata?.checkoutType === "product" &&
    ...
    if (!paymentMatchesCheckout) {
      return res
        ...
        .json({
          success: false,
          message: "Payment verification failed for this checkout session",
        });
    }
    ...
    try {
      const vendorIdsInCart = [
        ...new Set(cart.map((item) => item.vendor_id).filter(Boolean)),
      ];
      let shouldAutoConfirm = false;

      if (vendorIdsInCart.length > 0) {
        const vendorSettings = await Vendor.find({
          _id: { $in: vendorIdsInCart },
        })
        ...
        shouldAutoConfirm =
          ...
          vendorSettings.every((vendor) => vendor.auto_confirm_orders === true);
      }
      ...
      if (shouldAutoConfirm) {
        initialTimeline.push({
          status: "Confirmed",
          date: new Date(),
          description: "Order auto-confirmed by shop settings.",
        });
      }
        ...
        [
          {
            ...
            ...(shouldAutoConfirm ? { confirmed_at: new Date() } : {}),
          },
        ],
        { session },
      );
      ...
      const errorMessage = transactionError.message.includes(
        "Insufficient stock",
      )
...
const getProductWithRatings = async (req, res, next) => {
  try {
    ...
    const product = await Product.findById(productId).select(
      "id product_name product_type product_category product_description rating_average rating_count",
    );
